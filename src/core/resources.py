...
class ResourceManager:
	...
	def __init__(self):
		...
		self._resource_info = {
			ResourceType.COINS: ResourceInfo(
				name="Oro",
				symbol="🪙",
				description="Moneda básica del juego",
				color="#FFD700"
			),
	...
	def convert_resource(self, from_type: ResourceType, to_type: ResourceType, 
		...
		if self.spend_resource(from_type, amount):
			...
			logging.info(f"Convertido {amount} {from_type.value} → {actually_added} {to_type.value}")
			return True
	...
	def get_save_data(self):
		...
		return {
			"resources": {resource_type.value: amount for resource_type, amount in self._resources.items()}
		}
