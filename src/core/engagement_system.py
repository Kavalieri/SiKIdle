...
class EngagementSystem:
	...
	def can_claim_daily_reward(self, day: int) -> bool:
		...
		return (
			day <= self.current_streak and 
			reward_key not in self.daily_rewards_claimed
		)
	...
	def update_daily_goals(self):
		...
		if not self.goals_reset_date or datetime.fromisoformat(self.goals_reset_date).date() < today:
		...
		if 'daily_clicks' in self.daily_goals:
			...
			self.daily_goals['daily_clicks'].current_progress = min(
				total_clicks, 
				self.daily_goals['daily_clicks'].target_value
			)
		...
		if 'daily_buildings' in self.daily_goals and hasattr(self.game_state, 'building_manager'):
			...
			self.daily_goals['daily_buildings'].current_progress = min(
				buildings_today,
				self.daily_goals['daily_buildings'].target_value
			)
		...
		if 'daily_earnings' in self.daily_goals:
			...
			self.daily_goals['daily_earnings'].current_progress = min(
				current_coins,
				self.daily_goals['daily_earnings'].target_value
			)
	...
	def load_data(self):
		...
		try:
			...
			if result:
				(self.current_streak, self.last_login_date, rewards_claimed_str,
				 self.goals_reset_date, self.last_save_time) = result
