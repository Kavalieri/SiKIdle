...
class PrestigeManager:
	...
	def perform_prestige(self, total_coins: float) -> Dict[str, Any]:
		...
		logging.info(
			f"Prestigio realizado: +{crystals_gained} cristales, multiplicador: {self.income_multiplier:.2f}x"
		)
	...
	def load_data(self):
		...
		try:
			...
			if result:
				...
				if row:
					self.prestige_count, self.prestige_crystals, self.lifetime_coins = row

					# Recalcular multiplicadores
					self.income_multiplier = self.calculate_multiplier_from_crystals(
						self.prestige_crystals
					)
					self.click_multiplier = self.income_multiplier

					logging.info(
						f"Prestigio cargado: {self.prestige_crystals} cristales, {self.prestige_count} prestiges"
					)
