...
class LootCombatIntegration:
	...
	def __init__(
		...
		game_state=None,  # Para callbacks adicionales
	):
	...
	def _on_enemy_defeated(self, enemy_type: str, enemy_level: int, is_boss: bool) -> None:
		...
		logger.info(
			"*** LOOT COMBAT INTEGRATION *** Enemigo derrotado: %s (lvl %d), Boss: %s",
			enemy_type,
			enemy_level,
			is_boss,
		)
		...
		try:
			...
			# Si tenemos referencia a game_state, llamar también su método de registro
			if self.game_state and hasattr(self.game_state, "register_enemy_defeat"):
				self.game_state.register_enemy_defeat(enemy_type, enemy_level, is_boss)
	...
	def _notify_loot_obtained(self, item: "LootItem", enemy_type: str, is_boss: bool) -> None:
