...
class PremiumShopManager:
	...
	def get_shop_stats(self) -> Dict[str, Any]:
		...
		active_boost_count = len([b for b in self.active_boosts.values() 
								if time.time() < b['end_time']])
	...
	def load_data(self):
		...
		try:
			...
			if result:
				self.gems, cosmetics_str, convenience_str = result
			...
			logging.info(f"Premium shop loaded: {self.gems} gems, {len(self.active_boosts)} active boosts")
