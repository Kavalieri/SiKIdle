...
class CombatScreen(Screen):
	...
	def _switch_tab(self, tab_name):
		"""Cambia entre pestañas de combat y talentos."""
	...
	def _create_talents_area(self) -> BoxLayout:
		...
		combat_talents = [
			("critical_chance", "🍀 Probabilidad Crítica", "Aumenta la probabilidad de golpes críticos"),
	...
	def _create_talent_widget(self, talent_id, name, desc, available_points):
		...
		widget = BoxLayout(orientation='horizontal', size_hint_y=None, height=80, spacing=10, padding=[8, 4])
	...
	def _create_header(self) -> BoxLayout:
		...
		self.player_hp_bar = ProgressBar(
			max=self.player_max_hp,
			value=self.player_current_hp,
			size_hint_x=0.6
		)
		player_hp_container.add_widget(self.player_hp_bar)
		
		self.player_hp_label = Label(
			text=f"{self.player_current_hp}/{self.player_max_hp}",
			font_size='14sp',
			size_hint_x=0.2
		)
	...
	def _create_footer(self) -> BoxLayout:
		...
		status_label = Label(
			text="Haz click en el enemigo para atacar",
			size_hint_x=0.5,
			font_size='14sp'
		)
	...
	def _enemy_defeated(self):
		...
		if self.real_game_state:
			...
			self.real_game_state.resource_manager.add_resource(ResourceType.COINS, enemy.gold_reward)
	...
	def _enemy_attacks_player(self):
		...
		logging.debug(f"Player took {enemy_damage} damage. HP: {self.player_current_hp}/{self.player_max_hp}")
	...
	def _update_level_progress_display(self):
		...
		if self.level_progress_label:
			self.level_progress_label.text = f"Progreso: {self.enemies_defeated_in_level}/{self.enemies_needed_for_next_level}"
