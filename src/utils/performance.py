...
class BatchCalculator:
	...
	@staticmethod
	def calculate_building_production_batch(
		buildings: Dict, building_info: Dict, time_delta: float
	) -> Dict[str, float]:
		...
		for building_type, building in buildings.items():
			if building.count > 0:
				...
				base_production = info.base_production
				...
				if total_production > 0:
					production[info.production_resource] = total_production
...
class PerformanceOptimizer:
	...
	def optimize_building_production(self, building_manager, time_delta: float) -> Dict:
		...
		production = self.batch_calculator.calculate_building_production_batch(
			building_manager.buildings, building_manager.building_info, time_delta
		)
